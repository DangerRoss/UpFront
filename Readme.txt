...
UpFront.Buffers
Buffers provides a container type with a seekable write cursor over a fixed or growable block of memory and has direct access for copying.
The UpFront ArrayBuffer<T> effectively acheives the same role as ArrayBufferWriter<T> from System.Buffers.
...
UpFront.Pooling
Pooling provides a object pool for allowing reuse of objects. The events of a pooled object being initialised, reused and dropped by a pool
can be subscribed to using attributes on member functions of the class to assure safe state control.


UpFront.Events
Events provides a couple of simple Observables that are intended to be used in place of a 
.NET event handler when subscribing/unsubscribing to an event are on a critical path.
